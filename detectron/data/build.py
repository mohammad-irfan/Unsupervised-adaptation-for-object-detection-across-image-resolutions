...
from detectron.utils.logger import _log_api_usage, log_first_n
...
class AugMixDataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, i):
    a = copy.deepcopy(self.dataset[i])
    a["high_res"] = 1
    return self.dataset[i], a

  def __len__(self):
    return len(self.dataset)

def _train_loader_from_config(cfg, mapper=None, *, dataset=None, sampler=None, is_source=True):
    if dataset is None:
    ...
    if mapper is None:
        mapper = DatasetMapper(cfg, True, is_source = is_source)

...
def do_train(cfg, model, val_name, resume=False):
    ...
    for i in range(2,4):
        head.append(nn.Sequential(
                    ...
                    nn.Linear(feature_length[i]*feature_length[i+1], 128)
                ))
    ...
    with EventStorage(start_iter) as storage:
        for data,target_data, iteration in zip(data_loader, target_data_loader, range(start_iter, max_iter)):
            ...
            for i in range(4):
                head[i] = head[i].to(model.module.device)
                # print(up_sample)
                c_loss = consistency_criterion(pom["p"+str(i+2)], head[i], up_sample[i])
                loss_dict["consistency_loss"].append(abs(c_loss))
            loss_dict["consistency_loss"] = torch.mean(torch.stack(loss_dict["consistency_loss"]))
            ...
            if iteration - start_iter > 5 and (
                ...
                for writer in writers:
                    writer.write()
            del loss_dict, c_loss, pom, images, target_data
